    ...
    try:
        ...
        if ghost:
            record_ghost_ml_projection(
                normalized_symbol,
                normalized_timeframe,
                get_dashboard_candles(normalized_symbol, normalized_timeframe, safe_limit),
...
# ─────────────────────────────────────────────────────────────────────────────
# GHOST ML PHASE 1 — PROJECTION MEMORY + FUTURE-CANDLE EVALUATION
# Purpose:
# - Store ghost candle projections generated by /api/chart-overlays
# - Evaluate them once enough future candles exist
# - Produce accuracy stats before NRTR execution ML is introduced
# ─────────────────────────────────────────────────────────────────────────────
...
def record_ghost_ml_projection(
    ...
    projection_time = str(last_candle.get("time") or last_candle.get("timestamp") or "")
...
def evaluate_ghost_ml_records(symbol: str, timeframe: str, candles: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for index, candle in enumerate(candles):
        candle_time = str(candle.get("time") or candle.get("timestamp") or "")
        if candle_time:
...
def ghost_ml_summary(symbol: str = "MES1!", timeframe: str = "1m", limit: int = 500) -> Dict[str, Any]:
    normalized_symbol = normalize_symbol(symbol)
    normalized_timeframe = normalize_timeframe(timeframe)
    candles = get_dashboard_candles(normalized_symbol, normalized_timeframe, limit)
...
def ghost_ml_status(symbol: str = "MES1!", timeframe: str = "1m", limit: int = 500) -> Dict[str, Any]:
    return ghost_ml_summary(symbol, timeframe, limit)
...
def root() -> Dict[str, Any]:
    return {
        ...
        "engine": "main_v29_ghost_ml_phase1_memory",
        "endpoints": [
            "/api/latest-signal",
            "/api/recent-signals",
            "/api/recent-candles",
