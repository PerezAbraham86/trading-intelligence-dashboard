...
def timeframe_seconds(timeframe: Any = "1m") -> int:
    """Return the bucket size, in seconds, for a normalized chart timeframe.

    This is intentionally local to the live-feed bridge so SSE candle updates can
    be bucketed before they are cached/emitted. Without this, a 5m chart can get
    one new candle per websocket tick because each tick has a unique timestamp.
    """
    tf = normalize_timeframe(timeframe)
    ...
    try:
        quantity = int(tf[:-1] or "1")
        if tf.endswith("s"):
            return max(1, quantity)
        if tf.endswith("m"):
            return max(1, quantity) * 60
        if tf.endswith("h"):
            return max(1, quantity) * 3600
        if tf.endswith("d"):
            return max(1, quantity) * 86400
        if tf.endswith("w"):
            return max(1, quantity) * 604800
    except Exception:
        pass
...
def bucket_timestamp(timestamp: Any, timeframe: Any = "1m") -> int:
    """Floor a raw provider timestamp to the active chart timeframe bucket."""
...
def update_cache_from_candle(symbol: str, timeframe: str, candle: Dict[str, Any], raw_message: Optional[Dict[str, Any]] = None) -> None:
    ...
    # Replace any existing candle in the same timeframe bucket. This prevents
    # 5m/3m/10m charts from accumulating one candle per websocket tick.
    for index in range(len(history) - 1, -1, -1):
        history_bucket_time = int(
            history[index].get("time")
            or history[index].get("timestamp")
            or history[index].get("t")
            or 0
        )
    ...
    if not replaced:
        ...
        if len(history) > 1:
            previous_bucket_time = int(
                history[-2].get("time")
                or history[-2].get("timestamp")
                or history[-2].get("t")
                or 0
            )
