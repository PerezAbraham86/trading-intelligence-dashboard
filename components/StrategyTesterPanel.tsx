...
type BacktestResult = {
  ...
  equity: Array<{ time: number; value: number; tradeId: number; pnlPercent: number }>
  totalPnl: number
...
function toComparableCandles(candles: DashboardCandle[]) {
  return candles
    .map((candle) => ({ ...candle, numericTime: candleTimeToNumber(candle.time) }))
    .filter((candle) => candle.numericTime > 0 && Number.isFinite(candle.close))
    .sort((a, b) => a.numericTime - b.numericTime)
}

function calculateAtr(candles: ReturnType<typeof toComparableCandles>, length: number) {
  ...
  for (let index = 0; index < trueRanges.length; index += 1) {
    ...
    atrValues[index] = previous === null ? null : (previous * (safeLength - 1) + value) / safeLength
  }

  return atrValues
}

function calculateNrtrAtr(candles: ReturnType<typeof toComparableCandles>, settings: ChartStrategySettings): NrtrPoint[] {
  ...
  for (let index = 0; index < candles.length; index += 1) {
    ...
    if (atr === null || !Number.isFinite(atr)) {
      result.push({ time: candle.numericTime, value: null, direction: 0, buy: false, sell: false })
      continue
    }
...
function calculateNrtrPercentage(candles: ReturnType<typeof toComparableCandles>, settings: ChartStrategySettings): NrtrPoint[] {
  const result: NrtrPoint[] = []
  const coefficient = Math.max(0.001, Math.min(100, Number(settings.nrtrPercent) || 0.25)) / 100
...
function calculateNrtr(candles: ReturnType<typeof toComparableCandles>, settings: ChartStrategySettings) {
  if (settings.nrtrMode === 'Percentage') return calculateNrtrPercentage(candles, settings)
  return calculateNrtrAtr(candles, settings)
}

function calculateSmma(candles: ReturnType<typeof toComparableCandles>, settings: ChartStrategySettings): SmmaPoint[] {
  ...
  for (let index = 0; index < candles.length; index += 1) {
    ...
    else if (smma !== null) smma = (smma * (safeLength - 1) + candle.close) / safeLength

    const direction: 1 | -1 | 0 = smma === null ? 0 : candle.close >= smma ? 1 : -1
    result.push({
      time: candle.numericTime,
      value: smma,
      direction,
...
function findDirectionAt<T extends { time: number; direction: 1 | -1 | 0 }>(points: T[], time: number) {
  let direction: 1 | -1 | 0 = 0
...
function calculateBacktest(
  ...
  const mainPoints = strategyMode === 'NRTR' ? calculateNrtr(mainCandles, mainSettings) : calculateSmma(mainCandles, mainSettings)
  const miniOnePoints = strategyMode === 'NRTR' ? calculateNrtr(miniOneCandles, miniOneSettings) : calculateSmma(miniOneCandles, miniOneSettings)
  const miniTwoPoints = strategyMode === 'NRTR' ? calculateNrtr(miniTwoCandles, miniTwoSettings) : calculateSmma(miniTwoCandles, miniTwoSettings)
  ...
  for (let index = 1; index < mainCandles.length; index += 1) {
    ...
    if (openTrade && (oppositeLongExit || oppositeShortExit)) {
      ...
      const pnl = (exitPrice - openTrade.entryPrice) * side
      const pnlPercent = (pnl / openTrade.entryPrice) * 100
  ...
  if (openTrade && mainCandles.length > openTrade.entryIndex + 1) {
    ...
    const side = openTrade.side
    const pnl = (candle.close - openTrade.entryPrice) * side
    const pnlPercent = (pnl / openTrade.entryPrice) * 100
...
function EquityCurve({ equity }: { equity: BacktestResult['equity'] }) {
  if (!equity.length) {
    return (
      <div className="flex h-72 items-center justify-center rounded-xl border border-dark-700 bg-dark-900/50 text-sm text-gray-500">
        No backtest trades yet. Load all three charts and wait for enough candles.
      </div>
    )
  }
  ...
  const points = equity.map((item, index) => {
    const x = padding + (index / Math.max(1, equity.length - 1)) * (width - padding * 2)
  ...
  return (
    <div className="rounded-xl border border-dark-700 bg-black/30 p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="rounded-md border border-dark-600 px-2 py-1 text-[10px] text-gray-400">Preview</span>
      </div>
      <svg viewBox={`0 0 ${width} ${height}`} className="h-72 w-full overflow-visible">
        <line x1={padding} y1={zeroY} x2={width - padding} y2={zeroY} stroke="rgba(148,163,184,0.25)" strokeWidth="1" />
        <polyline fill="none" stroke="rgb(45,212,191)" strokeWidth="3" points={points} />
        {equity.map((item, index) => {
          const x = padding + (index / Math.max(1, equity.length - 1)) * (width - padding * 2)
          const y = height - padding - ((item.value - minValue) / range) * (height - padding * 2)
          return <circle key={item.tradeId} cx={x} cy={y} r="4" fill="rgb(45,212,191)" />
        })}
      </svg>
    </div>
  )
...
function MetricCard({ label, value, subValue, positive }: { label: string; value: string; subValue?: string; positive?: boolean }) {
  return (
    <div className="rounded-xl border border-dark-700 bg-dark-900/60 p-4 shadow-lg">
      ...
      <div className={`mt-2 text-xl font-bold ${positive === false ? 'text-red-300' : positive === true ? 'text-emerald-300' : 'text-gray-100'}`}>
        {value}
      </div>
...
export default function StrategyTesterPanel({
  ...
  const result = useMemo(
    ...
    [mainCandles, mainSettings, miniOneCandles, miniOneSettings, miniTwoCandles, miniTwoSettings, strategyMode]
  )
      ...
      <div className="mb-5 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">
            Candle-only preview using the three chart settings. SMC, Liquidity, Ghost, External Data, and ML are excluded.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className={`rounded-lg px-3 py-2 text-xs font-semibold ${activeTab === 'metrics' ? 'bg-white text-dark-900' : 'border border-dark-600 bg-dark-900 text-gray-300'}`}
          >
            Metrics
          </button>
          <button
            ...
            className={`rounded-lg px-3 py-2 text-xs font-semibold ${activeTab === 'trades' ? 'bg-white text-dark-900' : 'border border-dark-600 bg-dark-900 text-gray-300'}`}
          >
            List of trades
          </button>
        </div>
      ...
      {activeTab === 'metrics' ? (
        <div className="space-y-5">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
            ...
            <MetricCard
              label="Profit factor"
              value={result.profitFactor >= 999 ? '∞' : result.profitFactor.toFixed(3)}
                ...
                {result.trades.length === 0 ? (
                  ...
                  result.trades.slice().reverse().map((trade) => (
                    <tr key={trade.id}>
                      ...
                      <td className={`px-3 py-3 font-semibold ${trade.side === 'Long' ? 'text-emerald-300' : 'text-red-300'}`}>{trade.side}</td>
                      <td className="px-3 py-3">{formatDateTime(trade.entryTime)}</td>
                      <td className="px-3 py-3">{formatDateTime(trade.exitTime)}</td>
                      <td className="px-3 py-3 text-right">{trade.entryPrice.toFixed(2)}</td>
                      <td className="px-3 py-3 text-right">{trade.exitPrice.toFixed(2)}</td>
                      <td className={`px-3 py-3 text-right font-semibold ${trade.pnl >= 0 ? 'text-emerald-300' : 'text-red-300'}`}>{trade.pnl.toFixed(2)}</td>
                      <td className={`px-3 py-3 text-right ${trade.pnlPercent >= 0 ? 'text-emerald-300' : 'text-red-300'}`}>{formatPercent(trade.pnlPercent)}</td>
                      <td className="px-3 py-3 text-right text-emerald-300">{formatPercent(trade.mfePercent)}</td>
                      <td className="px-3 py-3 text-right text-red-300">{formatPercent(trade.maePercent)}</td>
                      <td className="px-3 py-3 text-right">{trade.barsHeld}</td>
                      <td className="px-3 py-3 text-gray-400">{trade.exitReason}</td>
                    </tr>
                  ))
                )}
              </tbody>
